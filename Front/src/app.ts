...
import { Fournisseur, Transaction, client } from "./types.js";
import { isTel, isNumCompte } from "./validator.js";
import { numero, nomComplet, port, fournisseurSelect, transactionSelect, montant, numeroDes, nomCompletDes, send, error, newNomComplet, newTel, adNewClient, numeroCompteAdd, nomCompletAdd, adNewCompte, messageCompte, messageClient, fournisseurSelect2, infoG, modalG, des, code, codeT, btnTransaction, list_transaction, list_compte, btnCompte, bloquer, debloquer, colfour, annulerTransa, selectFiltre, mon, infoTransaction, transactionDetail, montantRecu } from "./dom.js";
import { chargerSelect, viderChamps, fetche, message, createButton, afficherTransaction } from "./functions.js"
...
let id_expediteur: number | null = null;
let id_destinataire: number | null = null;
let mont: number | string | null = null;
let typ: string | null = null;
let nomComplete: string | null = null
    ...
    fetch('http://127.0.0.1:8000/api/transactions/historique', {
        ...
        selectFiltre.addEventListener("click", () => {
            if (selectFiltre.value == "montant") {
                data.sort((a: any, b: any) => a.montant - b.montant);
...
montantRecu.addEventListener('input', () => {
    ...
    let arecevoir: any
    let some: any = montantRecu.value;
    console.log(montant.value);
    if (fournisseurSelect2.value === "OM") {
        arecevoir = +some + (+some * 0.01)
    } else if (fournisseurSelect2.value === "WR") {
